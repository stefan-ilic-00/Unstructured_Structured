...
def findArUcoMarkers(frame, markerSize=4, totalMarkers=50, draw=True):
    ...
    #arucoDict = aruco.Dictionary_get(aruco.DICT_6x6_250), in order to have code more maintainable
    # key attribute is defined using information which was passed to the function
    key = getattr(aruco, f'DICT_{markerSize}X{markerSize}_{totalMarkers}')
    arucoDict = aruco.getPredefinedDictionary(key)

    # Creates aruco marker parameters
    arucoParam = aruco.DetectorParameters_create()
    # Detect aruco markers - pass image, send dictionary, then send aruco parameters with calibration and distortion matrix
    # It returns bounding boxes, IDs, and rejected IDs (no ID for it, but found it as a marker)
    ...
    # verify *at least* one ArUco marker was detected - if at least one bounding box exusts
    if len(bbox) > 0:
